...
from deepscm.experiments.brain_atrophy.synth.sem_vi.base_sem_experiment import BaseVISEM, MODEL_REGISTRY
...
class ConditionalVISEM(BaseVISEM):
    def __init__(self, **kwargs):
        ...
        # brain_volume flow
        brain_volume_net = DenseNN(3, [8, 16], param_dims=[1, 1], nonlinearity=torch.nn.LeakyReLU(.1))
    ...
    def pgm_model(self):
        ...
        # Brain volume node
        brain_context = torch.cat([sex, age_, score_], 1)
        ...
        ventricle_context = torch.cat([sex, age_, score_], 1)
        ...
        return age, score, sex, ventricle_volume, brain_volume

    @pyro_method
    def model(self,):
        age, score, sex, ventricle_volume, brain_volume = self.pgm_model() # TODO check if sex is working properly
        ...
        return x, z, age, score, sex, ventricle_volume, brain_volume
