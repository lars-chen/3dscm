...
from deepscm.experiments.brain_atrophy.base_experiment import BaseCovariateExperiment, BaseSEM, EXPERIMENT_REGISTRY, MODEL_REGISTRY  # noqa: F401
...
class BaseVISEM(BaseSEM):
    ...
    def __setattr__(self, name, value):
        ...
        if name == 'score_flow_lognorm_loc':
    ...
    @pyro_method
    def guide(self, x, age, score, sex, ventricle_volume, brain_volume):
        raise NotImplementedError()

    @pyro_method
    def svi_guide(self, x, age, score, sex, ventricle_volume, brain_volume):
        self.guide(x, age, score, sex, ventricle_volume, brain_volume)

    @pyro_method
    def svi_model(self, x, age, score, sex, ventricle_volume, brain_volume):
        with pyro.plate('observations', x.shape[0]):
            pyro.condition(self.model, data={'x': x, 'sex': sex, 'age': age, 'score': score, 'ventricle_volume': ventricle_volume, 'brain_volume': brain_volume})()
    ...
    @pyro_method
    def reconstruct(self, x, age, score, sex, ventricle_volume, brain_volume, num_particles: int = 1):
        obs = {'x': x, 'sex': sex, 'age': age, 'score': score, 'ventricle_volume': ventricle_volume, 'brain_volume': brain_volume}
        ...
        for _ in range(num_particles):
            ...
            recon, *_ = pyro.poutine.condition(
                self.sample, data={'sex': sex, 'age': age, 'score': score, 'ventricle_volume': ventricle_volume, 'brain_volume': brain_volume, 'z': z})(x.shape[0])
            recons += [recon]
