...
var_name = {'ventricle_vol': 'v', 'brain_vol': 'b', 'sex': 's', 'age': 'a'}
value_fmt = {
    'ventricle_vol': lambda s: rf'{float(s)/1000:.4g}\,\mathrm{{ml}}',
    'brain_vol': lambda s: rf'{float(s)/1000:.4g}\,\mathrm{{ml}}',
...
def plot_gen_intervention_range(model_name, interventions, idx, normalise_all=True, num_samples=32):
    ...
    suptitle = '$s={sex}; a={age}; b={brain_vol}; v={ventricle_vol}$'.format(
        **{att: value_fmt[att](orig_data[att].item()) for att in ('sex', 'age', 'brain_vol', 'ventricle_vol')}
    )
...
def interactive_plot(model_name):
    ...
    def plot(image, age, sex, brain_vol, ventricle_vol, do_age, do_sex, do_brain_vol, do_ventricle_vol):
        intervention = {}
        if do_age:
        ...
        if do_brain_vol:
            intervention['brain_vol'] = brain_vol * 1000.
        if do_ventricle_vol:
            intervention['ventricle_vol'] = ventricle_vol * 1000.
    ...
    w = interactive(plot, image=IntSlider(min=0, max=4, description='Image #'), age=FloatSlider(min=30., max=120., step=1., continuous_update=False, description='Age'),
              ...
              brain_vol=FloatSlider(min=800., max=1600., step=10., continuous_update=False, description='Brain Volume (ml):', style={'description_width': 'initial'}),
              do_brain_vol=Checkbox(description='do(brain_vol)'),
              ventricle_vol=FloatSlider(min=11., max=110., step=1., continuous_update=False, description='Ventricle Volume (ml):', style={'description_width': 'initial'}),
              do_ventricle_vol=Checkbox(description='do(ventricle_vol)'),)
