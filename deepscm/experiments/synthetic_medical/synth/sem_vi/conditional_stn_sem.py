...
class ConditionalSTNVISEM(BaseVISEM):
    def __init__(self, **kwargs):
        ...
        # decoder parts
        self.decoder = Decoder3D(num_convolutions=self.num_convolutions, filters=self.dec_filters, latent_dim=self.latent_dim + 2, upconv=self.use_upconv)
        ...
        # encoder parts
        self.encoder = Encoder3D(num_convolutions=self.num_convolutions, filters=self.enc_filters, latent_dim=self.latent_dim)

        # TODO: do we need to replicate the PGM here to be able to run conterfactuals? oO
        latent_layers = torch.nn.Sequential(torch.nn.Linear(self.latent_dim + 2, self.latent_dim), torch.nn.ReLU())
